...
- **CI cannot fetch YouTube captions or per-video metadata at all** — YouTube blocks cloud IPs regardless of cookies (verified Aug 30, 2026 with a fresh export). The workflow sets `CAPTION_FETCH=false` so runs skip the attempts. The **residential machine is the transcript path**: the `MarathonMeetings-RefreshTranscripts` task (7 AM / 7 PM) runs `fetch_transcript.py --all`, which upgrades displayed agenda-only meetings, matches BoardBook recordings and Kronenwetter SoundCloud audio (local Whisper, `small`), and gap-sweeps the channel listings for videos CI never created entries for (`GAP_SWEEP_DAYS`, default 45) — shipping `transcripts/<id>.meta.json` sidecars so CI's ingest never needs YouTube. Watchdogs: `skipped_videos.json` records when CI first saw each stuck video; one lingering past `STALE_VIDEO_HOURS` (36) opens a "🏠 Residential transcript fetcher may be down" issue (auto-closes). `YOUTUBE_COOKIES` is retained but currently unused — see `docs/cookie-refresh.md`.
